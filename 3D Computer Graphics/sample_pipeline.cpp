...
//    GLuint programID = LoadShaders( "../SimpleVertexShader.vertexshader", "../SimpleFragmentShader.fragmentshader" );
    GLuint pipelineObject = LoadPipeline("../SimpleVertexShader.vertexshader",
...
//        glUseProgram(programID);
        ...
        // glUseProgram() overrides glBindProgramPipeline()
        // Hence call glUseProgram(0) explicitly when using pipeline objects
        glUseProgram(0);
        glBindProgramPipeline(pipelineObject);
        ...
        // Step 0. Get the uniform variables for the pipeline
        GLint numUniforms = 0;
        glGetProgramInterfaceiv( programIDs[1], GL_UNIFORM, GL_ACTIVE_RESOURCES, &numUniforms );
        std::cout << "{Uniforms} #uniform variables : " << numUniforms << std::endl;

        GLenum  properties[] = { GL_NAME_LENGTH, GL_TYPE, GL_LOCATION, GL_BLOCK_INDEX };
        for (int nUniform = 0; nUniform < numUniforms; ++nUniform)
        {
            GLint  results[4];
            glGetProgramResourceiv( programIDs[1], GL_UNIFORM, nUniform, 4, properties, 4, NULL, results );

            GLint namesize = results[0] + 1;
            char *varName = new char[namesize];

            glGetProgramResourceName( programIDs[1], GL_UNIFORM, nUniform, namesize, NULL, varName );
            // glGetProgram

            std::cout <<"{uniforms} Name - " << varName << std::endl;
//            std::cout <<"{uniforms} Type - " << varName << std::endl;
        ...
        uboIndex = glGetUniformBlockIndex( programIDs[1], "LightArray" );
        if( uboIndex != GL_INVALID_INDEX )
        {
        
            // Step 2. Get the block size
            glGetActiveUniformBlockiv( programIDs[1],
                                  uboIndex,
            ...
            glGetUniformIndices(programIDs[1], 2, names, indices);
...
//            std::cout << "Indices : " << names[1] << ", " << indices[1] << std::endl;
            ...
            glGetActiveUniformsiv(programIDs[1], 2, indices,
...
//            std::cout << "Offset : " << names[1] << ", " << offset[1] << std::endl;
        ...
        GLint vTransformLoc = glGetUniformLocation( programIDs[0], "vertexTransform" );
...
//        glUniformMatrix4fv( vTransformLoc, 1, GL_FALSE, vTransform );
        glProgramUniformMatrix4fv( programIDs[0], vTransformLoc, 1, GL_FALSE, vTransform );
...
//        glUniformMatrix4fv( vTransformLoc, 1, GL_FALSE, vTransform );
    ...
    glDeleteProgramPipelines(1, &pipelineObject);
